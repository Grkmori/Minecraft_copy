	...
	public sealed class ResourcesLoader {
		public void LoadWorldResources() {
			...
			foreach(GameObject prefab in Resources.LoadAll<GameObject>("Prefabs/Dynamic/Entities/")) {
				bool tryAddFalse = WorldData.dictionaryDynamicEntitiesPrefabs.TryAdd(prefab.name, prefab);
				if(!tryAddFalse) {
					Debug.LogError("<b>Prefab " + prefab.name + "</b> <color=red><i>failed to add</i></color> to <b>dictionaryDynamicEntitiesPrefabs</b>.");
				}
			}
			if(WorldData.dictionaryDynamicEntitiesPrefabs.Count == 0) {
				Debug.LogWarning("<b>dictionaryDynamicEntitiesPrefabs</b> had some problems and <color=yellow><i>was not fully populated</i></color>.");
			} else {
				Debug.Log("<b>Prefabs</b> were successfully <i><color=green>loaded</color> and <color=cyan>populated</color></i> to the <b>dictionaryDynamicEntitiesPrefabs</b> with a total of <i>" + WorldData.dictionaryDynamicEntitiesPrefabs.Count + " Prefabs</i>.");
			}
