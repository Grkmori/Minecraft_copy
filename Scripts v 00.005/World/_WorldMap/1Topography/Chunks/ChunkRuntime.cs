...
namespace BlueBird.World.WorldMap.Topography.Chunks {
    public sealed class ChunkRuntime {
        ...
        /* Instances - For Chunks */
        Transform characterTransform;

        /* Storage - For Chunks */
        private static List<Vector3> listLastVisibleChunks = new List<Vector3>();
        private static List<Vector3> listVisibleChunks = new List<Vector3>();
        ...
        private readonly object lockUpdateChunkMeshPosition = new object();

        public void ManualStart() {
            VisualEventHandler.eventOnVisualTick_1 += QueueVisibleChunks;
            characterTransform = PlayerData.dictionaryPlayerObject["Character"].transform;
        }

        public void QueueVisibleChunks() {
            // Setting up
            ...
            if(playerChunkPosition.Equals(lastPlayerChunkPosition)) {
                ...
                for(float OffsetX = lastPlayerChunkPosition.x - visibleDistanceChunk; OffsetX <= lastPlayerChunkPosition.x + visibleDistanceChunk; OffsetX++) {
                    for(float OffsetZ = lastPlayerChunkPosition.z - visibleDistanceChunk; OffsetZ <= lastPlayerChunkPosition.z + visibleDistanceChunk; OffsetZ++) {
                        ...
                        if(!WorldMapData.dictionaryChunkData.ContainsKey(viewDistanceChunkPosition)) {
                            continue;
                        } else {
                            listVisibleChunks.Add(viewDistanceChunkPosition);
                            WorldMapData.dictionaryChunkData[viewDistanceChunkPosition].SetVisible(true);
                            ...
                            if(VisualData.dictionaryChunkMesh.ContainsKey(viewDistanceChunkPosition)) {
                                ...
                                queueUpdateChunkMeshData.Enqueue(WorldMapData.dictionaryChunkData[viewDistanceChunkPosition]);
                ...
                foreach(Vector3 vector in listLastVisibleChunks) {
                    if(listVisibleChunks.Contains(vector)) {
                        ...
                        WorldMapData.dictionaryChunkData[vector].SetVisible(false);
                    }
                }
        ...
        public void QueueChunkMeshData() {
            while(queueUpdateChunkMeshData.Count > 0) {
                lock(lockUpdateChunkMeshData) {
                    ...
                    if(tryPeekTrue) {
                        ...
                        if(!tryDequeueFalse) {
                            Debug.LogWarning("<b>ChunkData " + chunkData.chunkPosition + "</b> <color=yellow><i>TryDequeue failed</i></color> from <b>queueUpdateChunkMeshData</b>.");
                        } else {
                            ///Debug.Log("<b>ChunkData " + chunkData.chunkPosition + "</b> <color=green><i>TryDequeue successfully</i></color> and was <color=cyan><i>removed</i></color> from <b>queueChunkMeshUpdate</b>.");
                        }
        ...
        public void QueueChunkMesh() {
            while(queueUpdateChunkMeshPosition.Count > 0) {
                lock(lockUpdateChunkMeshPosition) {
                    // Generate/Regenerate ChunkMeshData for ChunkData
                    Vector3 chunkMeshPosition;
                    bool tryPeekTrue = queueUpdateChunkMeshPosition.TryPeek(out chunkMeshPosition);
                    if(tryPeekTrue) {
                        _chunkMeshRuntime.CreateChunkMesh(WorldMapData.dictionaryChunkObject[chunkMeshPosition], WorldMapData.dictionaryChunkData[chunkMeshPosition]);

                        bool tryDequeueFalse = queueUpdateChunkMeshPosition.TryDequeue(out chunkMeshPosition);
                        if(!tryDequeueFalse) {
                            Debug.LogWarning("<b>ChunkMeshPosition " + chunkMeshPosition + "</b> <color=yellow><i>TryDequeue failed</i></color> from <b>queueUpdateChunkMeshPosition</b>.");
                        } else {
                            ///Debug.Log("<b>ChunkMeshPosition " + chunkMeshPosition + "</b> <color=green><i>TryDequeue successfully</i></color> and was <color=cyan><i>removed</i></color> from <b>queueUpdateChunkMeshPosition</b>.");
                        }
                    } else {
