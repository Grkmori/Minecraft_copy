...
namespace BlueBird.World {
	public sealed class ResourcesLoader {
		public void LoadWorldMapResources() {
			foreach(GameObject prefab in Resources.LoadAll<GameObject>("Prefabs/Static/World")) {
				bool tryAddFalse = WorldMapData.dictionaryStaticWorldPrefabs.TryAdd(prefab.name, prefab);
				if(!tryAddFalse) {
					Debug.LogError("<b>Prefab " + prefab.name + "</b> <color=red><i>failed to add</i></color> to <b>dictionaryStaticWorldPrefabs</b>.");
				}
			}
			if(WorldMapData.dictionaryStaticWorldPrefabs.Count == 0) {
				Debug.LogWarning("<b>dictionaryStaticWorldPrefabs</b> had some problems and <color=yellow><i>was not fully populated</i></color>.");
			} else {
				Debug.Log("<b>Prefabs</b> were successfully <i><color=green>loaded</color> and <color=cyan>populated</color></i> to the <b>dictionaryStaticWorldPrefabs</b> with a total of <i>" + WorldMapData.dictionaryStaticWorldPrefabs.Count + " Prefabs</i>.");
			}
		}
		...
		public void LoadPlayerResources() {
			foreach(GameObject prefab in Resources.LoadAll<GameObject>("Prefabs/Player/Player")) {
				bool tryAddFalse = PlayerData.dictionaryPlayerPrefabs.TryAdd(prefab.name, prefab);
				if(!tryAddFalse) {
					Debug.LogError("<b>Prefab " + prefab.name + "</b> <color=red><i>failed to add</i></color> to <b>dictionaryPlayerPrefabs</b>.");
				}
			}
			if(PlayerData.dictionaryPlayerPrefabs.Count == 0) {
				Debug.LogWarning("<b>dictionaryPlayerPrefabs</b> had some problems and <color=yellow><i>was not fully populated</i></color>.");
			} else {
				Debug.Log("<b>Prefabs</b> were successfully <i><color=green>loaded</color> and <color=cyan>populated</color></i> to the <b>dictionaryPlayerPrefabs</b> with a total of <i>" + PlayerData.dictionaryPlayerPrefabs.Count + " Prefabs</i>.");
			}

			foreach(GameObject prefab in Resources.LoadAll<GameObject>("Prefabs/Player/UI/")) {
				bool tryAddFalse = PlayerData.dictionaryPlayerUIPrefabs.TryAdd(prefab.name, prefab);
				if(!tryAddFalse) {
			...
			if(PlayerData.dictionaryPlayerPrefabs.Count == 0) {
				...
				Debug.Log("<b>Prefabs</b> were successfully <i><color=green>loaded</color> and <color=cyan>populated</color></i> to the <b>dictionaryPlayerUIPrefabs</b> with a total of <i>" + PlayerData.dictionaryPlayerUIPrefabs.Count + " Prefabs</i>.");
			}
		}

		public void LoadEntityResources() {
			foreach(GameObject prefab in Resources.LoadAll<GameObject>("Prefabs/Dynamic/Entities/")) {
				bool tryAddFalse = EntityData.dictionaryDynamicEntityPrefabs.TryAdd(prefab.name, prefab);
				if(!tryAddFalse) {
					Debug.LogError("<b>Prefab " + prefab.name + "</b> <color=red><i>failed to add</i></color> to <b>dictionaryDynamicEntityPrefabs</b>.");
				}
			}
			if(EntityData.dictionaryDynamicEntityPrefabs.Count == 0) {
				Debug.LogWarning("<b>dictionaryDynamicEntityPrefabs</b> had some problems and <color=yellow><i>was not fully populated</i></color>.");
			} else {
				Debug.Log("<b>Prefabs</b> were successfully <i><color=green>loaded</color> and <color=cyan>populated</color></i> to the <b>dictionaryDynamicEntityPrefabs</b> with a total of <i>" + EntityData.dictionaryDynamicEntityPrefabs.Count + " Prefabs</i>.");
			}
		}
	}
}