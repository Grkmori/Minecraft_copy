...
namespace BlueBird.World.Data.InvisibleData.Chunks {
    public sealed class ChunkMeshRuntime {
        ...
        private bool voxelIsDrawn;
        private bool voxelIsTransparent;
        ...
        public void AddVoxelMeshDataToChunk(Vector3 @localPosition, Vector3 @chunkPosition, Chunk @chunkData, Voxel @voxelData) {
            // Setting up
            voxelIsDrawn = false;
            voxelIsTransparent = WorldData.dictionaryVoxelDefinition[@voxelData.voxelTypeName].isTransparent;
            ...
            if(voxelIsDrawn) {
                bool @tryAddFalse = @chunkData.dictionaryDrawnVoxels.TryAdd(@voxelData.voxelPosition, @voxelData);
                if(!@tryAddFalse) {
                    Debug.LogError("<b>Voxel " + @voxelData.voxelPosition + "</b> <color=red><i>failed to add</i></color> to <b>dictionaryDrawnVoxels</b>.");
                }
                if(@chunkData.dictionaryDrawnVoxels.Count < 1) {
                    Debug.LogWarning("<b>Adding Voxels</b> to the  <b>dictionaryDrawnVoxel</b> had some problems and <color=yellow><i>were not fully populated</i></color>. " +
                                     "A total of: <i>" + @chunkData.dictionaryDrawnVoxels.Count + " Voxels </i> were added to the <b>Chunk Dictionaries dictionaryDrawnVoxels</b>.");
                } else {
                    ///Debug.Log("<b>dictionaryDrawnVoxels</b> was <color=cyan><i>populated</i></color>. A total of: <i>" + @chunkData.dictionaryDrawnVoxels.Count + " Voxels </i>.");
                }
            }
        }
